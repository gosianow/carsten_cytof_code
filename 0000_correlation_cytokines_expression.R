...
prop <- lapply(1:length(path_bimatrix_frequencies), function(i){
  # i = 1
  prop <- read.table(path_bimatrix_frequencies[i], header = TRUE, sep = "\t", as.is = TRUE)
})

prop_out <- Reduce(function(...) merge(..., by = c("cluster", "label"), all=TRUE, sort = FALSE), prop)

prop_tmp <- t(prop_out[, a$sample])
colnames(prop_tmp) <- prop_out$label

prop <- data.frame(sample = a$sample, prop_tmp, check.names = FALSE)
...
ggadf <- merge(a, prop, by = "sample", sort = FALSE)
...
for(i in 1:(length(gglabels) - 1)){
  ...
  for(j in (i+1):length(gglabels)){
    ...
    for(d in c("data23", "data29")){
      
      ggp <- ggplot(ggadf[ggadf$data == d, , drop = FALSE]) +
        geom_point(aes_string(x = as.character(gglabels[i]), y = as.character(gglabels[j]), shape = "data_day", color = "group"), size = 3, alpha = 0.8) +
        geom_smooth(aes_string(x = as.character(gglabels[i]), y = as.character(gglabels[j]))) +
        theme_bw() +
        theme(axis.text = element_text(size = 14), 
      ...
      pdf(file.path(outdir, paste0(prefix, "frequencies_plot_corr_pairs_", gglabels[i], "_", gglabels[j], "_", d ,".pdf")), width = 7, height = 5)
      print(ggp)
      dev.off()
...
for(i in 1:length(corr_methods)){
  ...
  mat <- cor(ggadf[, gglabels], method = corr_methods[i], use = "complete.obs")
