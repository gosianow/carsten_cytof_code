...
prop <- lapply(1:length(path_frequencies), function(i){
  ...
  prop[, -which(colnames(prop) == "cluster")]
  
})

prop_out <- Reduce(function(...) merge(..., by = c("label"), all=TRUE, sort = FALSE), prop)
...
if(all(!complete.cases(prop_out))){
  ...
  prop_out <- prop_out[complete.cases(prop_out), , drop = FALSE]
}

prop_out$cluster <- 1:nrow(prop_out)
